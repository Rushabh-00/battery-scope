...
/** Calculates health from the design/reference capacity and the learned full-charge capacity. */
object BatteryHealthCalculator {
    ...
    fun calculate(designCapacityMah: Double?, fullChargeCapacitiesMah: List<Double>): Result {
        val design = designCapacityMah?.takeIf { it > 0.0 } ?: return Result(null, null, 0)
        val samples = fullChargeCapacitiesMah.takeLast(5).filter { it > 0.0 }
        if (samples.isEmpty()) return Result(null, null, 0)
        ...
        return Result(health, wear, samples.size)
    }
}
